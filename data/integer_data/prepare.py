...
Usage:
    python -m data.integer_data.prepare --input_file data.json --vocab_size 1000

Expected input format:
    - JSON file with list of sequences: [{"sequence": [0, 1, 2, 3, ...]}, ...]
    - CSV file with one sequence per row
    - NPY file with 2D array where each row is a sequence
"""
...
def main():
    parser = argparse.ArgumentParser(description="Prepare integer sequence dataset for GPT training")
    ...
    parser.add_argument("--bos_token", type=int, default=0, help="Beginning of sequence token")
    parser.add_argument("--num_shards", type=int, default=1, help="Number of training shards")
    parser.add_argument("--output_dir", type=str, help="Output directory (defaults to input file directory)")
